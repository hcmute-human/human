...
{
    public static IQueryable<T> SortOrDefault<T>(this IEnumerable<Orderable> orders, IQueryable<T> query, Func<IQueryable<T>, IOrderedQueryable<T>> sortDefault) where T : notnull
    {
        if (!orders.Any())
        {
